...
class _NotificationScreenState extends State<NotificationScreen> {
  List notificationArr = [];
  String? selectedOption = "All";
  int newNotificationCount = 0; // Declare the variable to store the count of new notifications
  ...
  Future<List<dynamic>> fetchNotifications() async {
    ...
    print("Body: ${response.body}");
    if (response.statusCode == 200) {
      final notifications = json.decode(response.body)['notification'];
      newNotificationCount = notifications.length; // Update the count
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          String avatarText = nObj['title'].split(' ')[0].toUpperCase();
          String formattedDate = DateFormat.yMd().format(createdDateTime);
          String formattedTime = DateFormat.jm().format(createdDateTime);
