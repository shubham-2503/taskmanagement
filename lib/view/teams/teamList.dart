...
class _TeamsFormedScreenState extends State<TeamsFormedScreen> {
  ...
  void initState() {
    super.initState();
    fetchMyTeams().then((_) {
      // Set _filteredTeams to be the same as _teams when data is fetched
      setState(() {
  ...
  void searchTeams(String query) {
    if (query.isEmpty) {
      ...
      setState(() {
        _filteredTeams = _teams; // Show all teams
      });
    } else {
      // If there is a search query, filter teams by task name
      setState(() {
        _filteredTeams = _teams
            .where((team) =>
